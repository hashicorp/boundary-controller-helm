...
#
# gke-integration-test.sh
# ---------------------------------------------------------------------------
# Validates that the boundary-controller Helm chart is correctly installed
# and active on a GKE cluster provisioned by the Terraform in this directory.
#
# Usage:
#   ./gke-integration-test.sh [OPTIONS]
#
# Options:
#   --project-id    PROJECT     GCP project ID   (or set GCP_PROJECT_ID)
#   --zone          ZONE        GKE cluster zone  (or set GKE_ZONE, default: us-central1-a)
...
if [ -z "${GCP_PROJECT_ID}" ]; then
    echo "ERROR: --project-id (or GCP_PROJECT_ID env var) is required."
    exit 1
fi

if [ -z "${GKE_CLUSTER_NAME}" ]; then
    echo "ERROR: --cluster-name (or GKE_CLUSTER_NAME env var) is required."
    exit 1
fi

# GKE kubectl context generated by: gcloud container clusters get-credentials
KUBE_CONTEXT="gke_${GCP_PROJECT_ID}_${GKE_ZONE}_${GKE_CLUSTER_NAME}"

echo -e "\n${BOLD}Boundary Controller — GKE Installation Validation${NC}"
...
for cmd in kubectl helm gcloud curl python3; do
...
# ---------------------------------------------------------------------------
# Section 2: GKE Cluster Connectivity
# ---------------------------------------------------------------------------
section "2. GKE Cluster Connectivity"

info "Fetching credentials for cluster '${GKE_CLUSTER_NAME}' (zone: ${GKE_ZONE})..."
if gcloud container clusters get-credentials "${GKE_CLUSTER_NAME}" \
        --zone "${GKE_ZONE}" \
        --project "${GCP_PROJECT_ID}" >/dev/null 2>&1; then
    pass "kubeconfig updated (context: ${KUBE_CONTEXT})"
else
    fail "Failed to fetch GKE credentials — check gcloud auth and cluster name"
fi
...
# Node count
NODE_COUNT=$(kubectl get nodes --context "${KUBE_CONTEXT}" \
...
# Services — api and cluster listeners
